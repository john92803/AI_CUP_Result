...
from yolov9.utils.general import check_img_size, check_requirements, non_max_suppression, \
    scale_boxes, xyxy2xywh, strip_optimizer, set_logging, increment_path
#from yolov9.utils.general import (LOGGER, Profile, check_file, colorstr, print_args, scale_boxes)
from yolov9.utils.plots import plot_one_box

#from yolov9.utils.plots import Annotator, colors, save_one_box
from yolov9.utils.torch_utils import select_device, time_synchronized
...
def detect(save_img=False):
    ...
    # Load model
    model = DetectMultiBackend(weights, device=device, dnn=False, data=opt.source, fp16=half, fuse=True)  # load FP32 model
    ...
    # classify = False
    # if classify:pdate(detections
    dataset = LoadImages(source, img_size=imgsz, stride=stride, auto=model.pt, vid_stride=1)
    ...
    for path, img, im0s, vid_cap, s in tqdm(dataset, desc=f'tracking {opt.name}'):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            detections = []
            if len(det):
                boxes = scale_boxes(img.shape[2:], det[:, :4], im0.shape)
